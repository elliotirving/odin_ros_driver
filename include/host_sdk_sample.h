...
public:
    ...
    void process_pair(const ImageConstPtr &rgb_msg, const PointCloud2ConstPtr &pcd_msg)
    {
        ...
        if (input_image_height > 0 && input_image_width > 0)
        {
            ...
            {
                *iter_res_x = rgbCloud_flat[4 * i];
                ++iter_res_x;
                *iter_res_y = rgbCloud_flat[4 * i + 1];
                ++iter_res_y;
                *iter_res_z = rgbCloud_flat[4 * i + 2];
                ++iter_res_z;
                *iter_res_rgb = rgbCloud_flat[4 * i + 3];
                ++iter_res_rgb;
            }
    ...
    void publishIntensityCloud(capture_Image_List_t *stream, int idx)
    {
        ...
        if (stream->imageCount == 4)
        {
            ...
            for (int i = 0; i < total_points; ++i)
            {
                if (confidence_data[i] < confidence_threshold)
                {
                    ...
                    *iter_confidence = 0;
                    ++iter_confidence;
                    *iter_offsettime = 0.0f;
                    ++iter_offsettime;
                }
                else
                {
                    // XYZ point
                    *iter_x = xyz_data_f[i * 3 + 2] / 1000.0f;
                    ++iter_x;
                    *iter_y = -xyz_data_f[i * 3 + 0] / 1000.0f;
                    ++iter_y;
                    *iter_z = xyz_data_f[i * 3 + 1] / 1000.0f;
                    ++iter_z;

                    *iter_intensity = intensity_data[i];
                    ++iter_intensity;
                    *iter_confidence = confidence_data[i];
                    ++iter_confidence;
        ...
        else
        {
            ...
            for (int i = 0; i < total_points; ++i)
            {
                *iter_x = xyz_data_f[i * 4 + 2] / 1000.0f;
                ++iter_x;
                *iter_y = -xyz_data_f[i * 4 + 0] / 1000.0f;
                ++iter_y;
                *iter_z = xyz_data_f[i * 4 + 1] / 1000.0f;
                ++iter_z;
...
// Publish point cloud
    ...
    void publishRgb(capture_Image_List_t *stream)
    {
        ...
        { // new version jpeg data
            ...
            if (m_undistort_map_init_success)
            {
                ...
                cv_undistorted_image.header.frame_id = "rgb";
            }
    ...
    void publishWiwc(capture_Image_List_t *stream)
    {
        ...
        wiwc_publisher_.publish(msg);
#endif
        // Cache T_CL and T_IL for use in publishOdometry
        for (int i = 0; i < 16; ++i)
        {
            cached_T_CL_(i / 4, i % 4) = msg.pose.covariance[i];
            cached_T_IL_(i / 4, i % 4) = msg.twist.covariance[i];
        }
        wiwc_valid_ = (cached_T_CL_ - Eigen::Matrix4d::Identity()).norm() > 1e-6 &&
                      (cached_T_IL_ - Eigen::Matrix4d::Identity()).norm() > 1e-6;
    }
    ...
    void publishOdometry(capture_Image_List_t *stream, OdometryType odom_type, bool show_path, bool show_camerapose)
    {
        ...
        case OdometryType::STANDARD:
        {
            ...
            // Publish T_world_camera using online-refined extrinsics from wiwc
            {
                Eigen::Vector3d P_cam(msg.pose.pose.position.x,
                                      msg.pose.pose.position.y,
                                      msg.pose.pose.position.z);
                Eigen::Quaterniond R_imu(msg.pose.pose.orientation.w,
                                         msg.pose.pose.orientation.x,
                                         msg.pose.pose.orientation.y,
                                         msg.pose.pose.orientation.z);
                if (wiwc_valid_)
                {
                    Eigen::Matrix4d T_IC = cached_T_IL_ * cached_T_CL_.inverse();
                    P_cam = P_cam + R_imu * T_IC.block<3, 1>(0, 3);
                    R_imu = R_imu * Eigen::Quaterniond(T_IC.block<3, 3>(0, 0));
                }
                else if (extrinsic_ok_)
                {
                    P_cam = P_cam + R_imu * t_ic_;
                    R_imu = R_imu * R_ic_;
                }
                R_imu.normalize();
                auto cam_msg = nav_msgs::msg::Odometry();
                cam_msg.header = msg.header;
                cam_msg.child_frame_id = "rgb";
                cam_msg.pose.pose.position.x = P_cam.x();
                cam_msg.pose.pose.position.y = P_cam.y();
                cam_msg.pose.pose.position.z = P_cam.z();
                cam_msg.pose.pose.orientation.w = R_imu.w();
                cam_msg.pose.pose.orientation.x = R_imu.x();
                cam_msg.pose.pose.orientation.y = R_imu.y();
                cam_msg.pose.pose.orientation.z = R_imu.z();
                camera_odom_publisher_->publish(cam_msg);
            }
...
private:
    ...
    Eigen::Matrix4d cached_T_IL_{Eigen::Matrix4d::Identity()};
    bool wiwc_valid_{false};
    Eigen::Matrix4d T_il_ = (Eigen::Matrix4d() << 1.0, 0.0, 0.0, 0.00347,
                             0.0, 1.0, 0.0, 0.03447,
                             0.0, 0.0, 1.0, 0.02174,
                             0.0, 0.0, 0.0, 1.0)
                                .finished();
    ...
    void initialize_publishers()
    {
        ...
        camera_odom_publisher_ = node_->create_publisher<ros::Odometry>("odin1/odometry_camera", qos_small);
    ...
    void initialize_publishers(ros::NodeHandle &nh)
    {
        ...
        camera_odom_publisher_ = nh.advertise<ros::Odometry>("odin1/odometry_camera", 100);
    ...
    rclcpp::Publisher<ros::Odometry>::SharedPtr camera_odom_publisher_;
